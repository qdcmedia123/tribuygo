...
$collection->createIndex(["title" => "text", "keyword" => "text"]);
...
[
    ...
    foreach ($data as $item) {
        ...
        $content = curl_get_file_contents($item['url']);

        if($content === '' || empty($content) || $content === false) {
    ...
    for ($i = 0; $i < $getMaxRecord; $i++) {
      foreach ($getData as $key => $value) {
        if (isset($getData[$key][$i])) {
            $b[$key] = $getData[$key][$i];
            $b[$key]['website'] = $key;
            $b[$key]['keyword'] = urldecode(strtolower($keyword));
...
foreach($hits as $item) {
    ...
    $item['ratings'] = '';
    $item['review'] = '';
     $item['keyword'] = urldecode(strtolower($search));
